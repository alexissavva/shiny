  ...
  shinyApp(
    ...
    server <- function(input, output,session) {
      ...
      output$distPlot2 <- renderPlot({
        if (univarie$type == 'quant') {
          plot(
            ...
            main = paste("Distribution de" , input$choice_univar)
          )
        }
        else {
      ...
      observeEvent(input$launch_pred, {
        ...
        output$obs_prev_lm <- renderPlot( plot_grid(plotlist = plots_train_list[1:2], ncol = 2),width = 600)  # 
      ...
      observeEvent({input$model_res
        input$set_right},
        {
          ...
          {
            output$obs_prev_lm <- renderPlot( plot_grid(plotlist = plots_train_list_global[(2*dict_index[[input$model_res]]-1) :(2*dict_index[[input$model_res]])], ncol = 2), width = 600)  
          }
          else
          {
            output$obs_prev_lm <- renderPlot( plot_grid(plotlist = plots_test_list_global[(2*dict_index[[input$model_res]]-1) :(2*dict_index[[input$model_res]])], ncol = 2), width = 600) 
          }
        }
